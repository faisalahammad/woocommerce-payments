...
class WC_Payments_Admin_Banner_Test extends WCPAY_UnitTestCase {
	...
	public function test_init_hooks_registers_sections_hook_for_active_tab(): void {
		...
		$this->assertNotFalse(
			has_action( 'woocommerce_sections_general', [ $banner, 'maybe_show_banners' ] )
		);

		remove_action( 'woocommerce_sections_general', [ $banner, 'maybe_show_banners' ] );
	...
	public function test_init_hooks_registers_sections_hook_for_checkout_tab(): void {
		...
		$this->assertNotFalse(
			has_action( 'woocommerce_sections_checkout', [ $banner, 'maybe_show_banners' ] )
		);

		remove_action( 'woocommerce_sections_checkout', [ $banner, 'maybe_show_banners' ] );
		unset( $_GET['page'], $_GET['tab'] );
	}
