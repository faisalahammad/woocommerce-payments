...
class WC_Payments_Admin_Banner {
	...
	public function init_hooks(): void {
		...
		if ( isset( $_GET['page'] ) && 'wc-settings' === sanitize_key( wp_unslash( $_GET['page'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			add_action( "woocommerce_sections_{$tab}", [ $this, 'maybe_show_banners' ] );
		}
	}

	/**
	 * Renders every eligible banner on a WooCommerce settings tab.
	 *
	 * @return void
	 */
	public function maybe_show_banners(): void {
		$this->maybe_show_test_to_live_notice();
		$this->maybe_show_post_kyc_activation_notice();
	}
